...
class Weight2Go(wx.Frame):
    def __init__(self):
        super().__init__(None, title="Weight2Go", size=(750, 650))

        panel = wx.Panel(self)
        notebook = wx.Notebook(panel)

        self.tab_input = wx.Panel(notebook)
        self.tab_graph_tips = scrolled.ScrolledPanel(notebook, style=wx.TAB_TRAVERSAL)
        self.tab_graph_tips.SetupScrolling()

        notebook.AddPage(self.tab_input, "Input")
        notebook.AddPage(self.tab_graph_tips, "Graph + Tips")

        self.build_tab_input(notebook)
        self.build_tab_graph_tips()
    ...
    def build_tab_input(self, notebook):
        panel = self.tab_input
        panel.SetBackgroundColour("#511C1C")

        vbox = wx.BoxSizer(wx.VERTICAL)

        title = wx.StaticText(panel, label="Welcome to Weight2Go!")
    ...
    def build_tab_graph_tips(self):
        self.tab_graph_tips.SetBackgroundColour("#511C1C")
        vbox = wx.BoxSizer(wx.VERTICAL)

        heading = wx.StaticText(self.tab_graph_tips, label="BMI Graph & Personalized Tips")
        ...
        self.canvas = FigureCanvas(self.tab_graph_tips, -1, self.figure)
        vbox.Add(self.canvas, 0, wx.EXPAND | wx.ALL, 15)

        self.lbl_message = wx.StaticText(self.tab_graph_tips, label="Your friendly message will appear here.")
        ...
        tips_heading = wx.StaticText(self.tab_graph_tips, label="Health Tips")
        ...
        self.lbl_tips = wx.StaticText(self.tab_graph_tips, label="Health Tips will appear here.")
        ...
        self.tab_graph_tips.SetSizer(vbox)
